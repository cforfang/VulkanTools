...
static char *VkFormatString(VkFormat fmt) {
    switch (fmt) {
#define STR(r)          \
        case VK_FORMAT_##r: \
            return #r
...
static void writePPM(const char *filename, VkImage image1) {
    ...
    */
    ...
    /*
    char *fmt = VkFormatString(format);

    printf("\nFORMAT %s:\n", fmt);

    char *token;
    char dstFormat[20] = "VK_FORMAT_R8G8B8A8_";
    char *rest = fmt;
    
    while ((token = strtok_r(rest, "_", &rest)))
        printf("%s\n", token);

//#define STR(token) VK_FORMAT_R8G8B8A8_##token; 
    
    VkFormat destFormat = VK_FORMAT_R8G8B8A8_UNORM;
    
  //  printf("%d\n", STR(token));
    if(!strcmp(token, "UNORM")) {
        destFormat = VK_FORMAT_R8G8B8A8_UNORM;
    } else if (!strcmp(token, "SNORM")) {
        destFormat = VK_FORMAT_R8G8B8A8_SNORM;
    } else if (!strcmp(token, "USCALED")) {
        destFormat = VK_FORMAT_R8G8B8A8_USCALED;
    } else if (!strcmp(token, "SSCALED")) {
        destFormat = VK_FORMAT_R8G8B8A8_SSCALED;
    } else if (!strcmp(token, "UINT")) {
        destFormat = VK_FORMAT_R8G8B8A8_UINT;
    } else if (!strcmp(token, "SINT")) {
        destFormat = VK_FORMAT_R8G8B8A8_SINT;
    } else if (!strcmp(token, "SRGB")) {
        destFormat = VK_FORMAT_R8G8B8A8_SRGB;
    }
    */
